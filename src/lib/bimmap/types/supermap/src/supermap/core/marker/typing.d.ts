namespace BimMap {
  namespace Supermap{
    interface IDomMarker extends BimMap.IDispose {
      /**
       * 修改覆盖物
       * @param {Cesium.DivOverlay} overlay 覆盖物对象
       * @param {IOverlayOptions | IOverlayOptions[]} options 创建点的配置
       */
      update(overlay: Cesium.DivOverlay, options: IOverlayOptions): this;
      /**
       * 创建覆盖物
       *
       * @param {IOverlayOptions | IOverlayOptions[]} options 创建点的配置
       ...
       *
       * @param {State.BaseStateEventCallback<"supermap">} callBack 回调函数
       */
      addClick(callBack: State.BaseStateEventCallback<"supermap">): this;
    }
    var DomMarker: IServiceConstructor<"supermap", IOverlayOptions, IDomMarker>;

    interface IIconMarker
      extends IFeatureHandle<"supermap">,
        State.IBaseState<"supermap", ServiceOptions.IIconMarkerOptions> {
      /**
