  ...
  namespace Leaflet {
    interface IGridPolygon
      extends IFeatureHandle<"leaflet">,
        State.IBaseState<"leaflet", ServiceOptions.IGridPolygon> {
      /**
       ...
       */
      addRightClick(
        callBack: BaseStateEventCallback<"leaflet">,
        name: string
      ): this;
      /**
       * @description 获取 Feature 的中心点
       *
       * @param {IBimMapHandle["leaflet"]["feature"]} feature 传入的 Feature 实例
       * @return {Number[]} 返回一个经纬度数组
       */
      getCenterCoordinates(
        feature: IBimMapHandle["leaflet"]["feature"]
      ): LngLatType;
