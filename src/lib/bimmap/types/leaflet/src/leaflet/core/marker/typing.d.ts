namespace BimMap {
  namespace Leaflet {
    interface IDomMarker extends BimMap.IDispose {
      /**
       * 修改覆盖物
       * @param {L.DivOverlay} overlay 覆盖物对象
       * @param {IOverlayOptions | IOverlayOptions[]} options 创建点的配置
       */
      update(overlay: L.DivOverlay, options: IOverlayOptions): this;
      /**
       * 创建覆盖物
       *
       * @param {IOverlayOptions | IOverlayOptions[]} options 创建点的配置
       ...
       *
       * @param {State.BaseStateEventCallback<"leaflet">} callBack 回调函数
       */
      addClick(callBack: State.BaseStateEventCallback<"leaflet">): this;
    }
    var DomMarker: IServiceConstructor<"leaflet", IOverlayOptions, IDomMarker>;

    interface IIconMarker
      extends IFeatureHandle<"leaflet">,
        State.IBaseState<"leaflet", ServiceOptions.IIconMarkerOptions> {
      /**
